...
food.get("/insights", async (c) => {
    ...
    const logs = await db
        .select()
        .from(foodLogs)
        .where(and(eq(foodLogs.userId, userId), gte(foodLogs.loggedAt, thirtyDaysAgo)))
        .all();
    if (logs.length === 0) {
        return c.json({
            patterns: [],
    ...
    let totalCalories = 0;
    let lateCalories = 0;
    const breakfastProteinByDay = {};
    const weekdayCaloriesByDay = {};
    const weekendCaloriesByDay = {};
    ...
    const lateCaloriePct = totalCalories > 0 ? Math.round((lateCalories / totalCalories) * 100) : 0;
    const breakfastDayKeys = Object.keys(breakfastProteinByDay);
    const avgBreakfastProtein = breakfastDayKeys.length > 0
        ? Math.round(Object.values(breakfastProteinByDay).reduce((a, b) => a + b, 0) /
    ...
    const avgWeekdayCalories = weekdayDayKeys.length > 0
        ? Math.round(Object.values(weekdayCaloriesByDay).reduce((a, b) => a + b, 0) /
            weekdayDayKeys.length)
        : 0;
    const avgWeekendCalories = weekendDayKeys.length > 0
        ? Math.round(Object.values(weekendCaloriesByDay).reduce((a, b) => a + b, 0) /
            weekendDayKeys.length)
        : 0;
    const hasWeekComparison = weekdayDayKeys.length >= 3 && weekendDayKeys.length >= 2;
    const patterns = [
        {
            id: "late_calories",
        ...
        },
        {
            ...
            detail: breakfastDayKeys.length >= 3
                ? `Average ${avgBreakfastProtein}g of protein per breakfast`
                : "Not enough breakfast logs to detect a pattern",
        },
        {
            id: "weekend_calories",
            detected: hasWeekComparison && avgWeekendCalories > avgWeekdayCalories * 1.2,
